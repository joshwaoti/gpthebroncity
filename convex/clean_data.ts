...
export const updateAllBlogs = mutation({
    args: {},
    handler: async (ctx) => {
        let updated = 0;
        ...
        return `Updated ${updated} blog posts with formatted content`;
    },
});

export const cleanBlogContent = mutation({
    args: {},
    handler: async (ctx) => {
        ...
        for (const blog of blogs) {
            if (blog.content && blog.content.includes("*")) {
                const cleanedContent = blog.content.replace(/\*+/g, '');
                await ctx.db.patch(blog._id, { content: cleanedContent });
                updated++;
            }
        }
        
        return `Cleaned ${updated} blog posts`;
    },
});

export const cleanEventDescriptions = mutation({
    args: {},
    handler: async (ctx) => {
        ...
        for (const event of events) {
            if (event.description) {
                ...
                if (desc.includes("automatically imported")) {
                    await ctx.db.patch(event._id, { description: "" });
                    updated++;
                }
            }
        }
        
        return `Cleaned ${updated} event descriptions`;
    },
});
